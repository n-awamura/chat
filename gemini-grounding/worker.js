...
async function callGeminiApi(prompt, modelName, retryCount = 0) {
  ...
  /*
  // 🔁 将来的に modelName を使って model-switcher 経由で呼び出す場合の例：
...
// まとめ用のGemini呼び出し：gemini-model-switcher経由でgemini-2.0-flashを利用
...
async function callGemini() {
  ...
  // 1️⃣ 質問部分はbuildPromptFromHistoryで全会話履歴を取得
  const history = buildPromptFromHistory();
  if (!history) return;

  // 2️⃣ Cloudflare Worker（グラウンディング）へ送信（シンプルなプロンプトとして最後のユーザー発言も利用）
  ...
  // 3️⃣ 会話履歴全体を利用して、語尾補正などの後処理プロンプトを作成
...
${history}`;
  ...
  // 🔗 ソース（grounding結果）があるなら表示
  if (response.sources?.length) {
    const chatMessagesDiv = document.getElementById('chatMessages');
    const sourcesDiv = document.createElement('div');
    sourcesDiv.classList.add('bubble-text');
    sourcesDiv.style.margin = '10px';
    sourcesDiv.style.fontSize = '0.9em';
    sourcesDiv.innerHTML = `<strong>参考リンク：</strong><ul>${response.sources.map(src => `<li>${src}</li>`).join('')}</ul>`;
    chatMessagesDiv.appendChild(sourcesDiv);
  }
...
async function summarizeSessionAsync(session) {
  ...
  const summary = summaryObj?.answer;
